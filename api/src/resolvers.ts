...
const resolvers = {
    Query: {
        ...
        deploymentStatus: async (_: any, { id }: { id: number }, { prisma, userId }: any) => {
            if (!userId) {
                throw new Error('Not authenticated.');
            }
    ...
    Mutation: {
        ...
        deployProject: async (
            ...
            try {
                ...
                deployment = await prisma.deployment.create({ // Assign to declared variable
                    data: {
                        projectId: projectId,
                        status: 'pending',
                        version: 'TBD',
                ...
                console.log(`Triggering asynchronous deployment process for deployment ${deploymentId}.`);
                ...
                enqueueDeployment(async () => {
                    try {
                        ...
                        console.error(`[Deployment ${deploymentId}] Unhandled error from processDeployment:`, error);
                        // Optionally update status to failed here if not already done by processDeployment
                         await prisma.deployment.update({
                             where: { id: deploymentId },
                             data: {
                                 status: 'failed',
                                 errorMessage: `Unhandled processing error: ${error instanceof Error ? error.message : String(error)}`
                             }
                         }).catch((dbError:any) => console.error(`[Deployment ${deploymentId}] Failed to update status to failed after unhandled error:`, dbError));
                    }
                });

                console.log(`Asynchronous deployment process triggered for deployment ${deploymentId}.`);


                // 3. Immediately return the pending deployment record
                console.log(`Returning pending deployment record ${deploymentId} to client.`);
                return deployment;


            }
            catch (error: any) {
                console.error(`Error during initial deployment request for project ${projectId}:`, error.message);
                ...
                if (typeof deployment !== 'undefined' && deployment !== null) {
                     console.log(`Updating deployment ${deployment.id} status to 'failed' due to initiation error.`);
                     await prisma.deployment.update({
                         where: { id: deployment.id },
                         data: { status: 'failed', errorMessage: error.message },
                     }).catch((dbError:any) => console.error(`Failed to update deployment ${deployment.id} to failed status:`, dbError));
                }
